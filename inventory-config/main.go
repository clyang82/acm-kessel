...
var schemaDir = "schema/resources"
...
// NormalizeResourceType normalizes the resource type name
...
func main() {
	...
	if err := preloadSchemas(); err != nil {
		log.Errorf("Error preloading schemas: %v", err)
		os.Exit(1)
	}

	// Save schema cache
	if err := saveSchemaCache(); err != nil {
		log.Errorf("Error saving schema cache: %v", err)
		os.Exit(1)
	}
